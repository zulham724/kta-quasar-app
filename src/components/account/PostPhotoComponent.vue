...
<div>
    ...
    <div class="row q-pt-md">
        <div class="col-4" style="padding:0.4vw" v-for="post in Auth.auth.posts.filter(post => post.files.length && post.files[0].value!='document')" :key="post.id" @click="$router.push(`/user/photolist/${Auth.auth.id}/${post.id}`)">
