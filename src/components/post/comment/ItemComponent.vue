...
<div>
    <q-item clickable v-ripple v-if="comment != null">
        ...
        <q-item-section side top class="q-pt-md">
            <q-btn flat size="sm" round :color="comment.liked_count ? 'red' : null" :icon="comment.liked_count ? 'favorite' : 'favorite_border'" @click="comment.liked_count ? dislike(comment.id) : like(comment.id)" />
        </q-item-section>
    </q-item>
</div>
</template>
