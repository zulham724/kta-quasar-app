        ...
        <q-pull-to-refresh @refresh="getUser" color="teal">
            <div class="q-pt-sm row items-start q-gutter-sm">
                <div v-if="loading" style="width:100%">
                    <q-card flat bordered class="q-mt-sm" style="width:100%" v-for="n in 3" :key="`loading-${n}`" f>
                        <q-item>
                ...
                <q-intersection v-for="post in user.posts.filter(
              item => item.is_public
            )" :key="post.id" :style="`min-height: 80vh;width: 100vw`" :id="`post${post.id}`">
