    ...
    <q-page v-if="user != null">
        <q-pull-to-refresh @refresh="getUser" color="teal">
            <div class="q-pt-sm row items-start q-gutter-sm">
                ...
                <q-intersection v-for="post in user.posts" :key="post.id" :style="`min-height: 80vh;width: 100vw`" :id="`post${post.id}`">
            ...
            return new Promise((resolve, reject) => {
                this.$store
                    ...
                    .then(res => {
                        this.user = {
                            ...res.data,
                            posts: res.data.posts.filter(item => item.files.length && item.files[0].value != 'document')
                        };
                        resolve(res);
                        if (done) done();
                    })
